...
class SettingsDialog(QDialog):
    ...
    def init_playback_tab(self):
        ...
        self.playback_stream_profile_combo = QComboBox()
        self.playback_stream_profile_combo.addItem(_("Kualitas Tinggi YouTube"), "hq")
        self.playback_stream_profile_combo.addItem(_("Seimbang (Disarankan)"), "balanced")
        self.playback_stream_profile_combo.addItem(_("Stabil untuk Jaringan Lambat"), "stable")
        self.playback_stream_profile_combo.addItem(_("Hemat Bandwidth Maksimum"), "data_saver")
        self.playback_stream_profile_combo.setToolTip(
            _("Atur prioritas kualitas atau stabilitas stream playback sesuai kondisi jaringan Anda.")
        )
        saved_profile = self.current_settings.get('playback_stream_profile', 'balanced')
        profile_index = self.playback_stream_profile_combo.findData(saved_profile)
        self.playback_stream_profile_combo.setCurrentIndex(profile_index if profile_index >= 0 else 0)
        playback_profile_label = QLabel(_("Profil Stabilitas Stream:"))
        playback_profile_label.setBuddy(self.playback_stream_profile_combo)
        layout.addRow(playback_profile_label, self.playback_stream_profile_combo)
    ...
    def accept_settings(self):
        ...
        self.current_settings['playback_stream_profile'] = self.playback_stream_profile_combo.currentData()
