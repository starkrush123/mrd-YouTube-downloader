...
class SearchHandler:
    ...
    def process_input(self):
        ...
        if not txt:
            QMessageBox.warning(self.main_window, _("Input Kosong"), _("Masukkan URL atau kata kunci."))
            return
        ...
        if self.main_window.is_youtube_channel_url(txt):
            QMessageBox.information(
                self.main_window,
                _("URL Channel Terdeteksi"),
                f"{_('URL')} '{txt[:60]}...' {_('dikenali sebagai channel. Akan menampilkan 100 video terbaru.')}",
            )
            self.start_channel_item_fetch(txt)
        elif self.main_window.is_potential_playlist_url(txt):
            QMessageBox.information(
                self.main_window,
                _("URL Playlist Terdeteksi"),
                f"{_('URL')} '{txt[:60]}...' {_('dikenali sebagai playlist. Akan menampilkan isinya.')}",
            )
    ...
    def start_search(self, query, search_type=None):
        if any(
            (t and t.isRunning())
            for t in [
                self.main_window.download_thread,
                self.main_window.playlist_fetch_thread,
                self.main_window.channel_fetch_thread,
            ]
        ):
        ...
        if search_type is None:
            search_type = self.main_window.main_view_widget.search_type_combo.currentText()

        self.main_window.search_results = []
        self.main_window.related_seed_url = None
        self.main_window.current_results_context = "none"

        if search_type == _("Channel"):
            self.start_channel_item_fetch(query)
            return
        if search_type == _("Video"):
            self.main_window.current_results_context = "video_search"
        elif search_type == _("Playlist"):
            self.main_window.current_results_context = "playlist_search_results"

        self.main_window.set_ui_busy_state(True, "search")
        lbl = _("Video") if search_type == _("Video") else _("Playlist")
        self.main_window.set_status_text(f"{_('Mencari')} {lbl} \"{query}\"...")
        self.main_window.update_window_title_status(f"{_('Mencari')} {lbl} ({query[:20]}...)")
        ...
        self.main_window.operation_progress_dialog = OperationProgressDialog(f"{_('Mencari')} {lbl}: {query[:30]}...", self.main_window)
        self.main_window.operation_progress_dialog.show()

        cookie_params = self._get_cookie_params()
        self.main_window.search_thread = SearchThread(
            query, self.main_window.settings.get("search_results_count", 10), search_type, cookie_params, self.main_window
        )
    ...
    def handle_search_results_batch(self, results, result_type):
        ...
        if self.active_search_results_dialog and self.active_search_results_dialog.isVisible():
            ...
            entity = _("video") if result_type == "video" else _("playlist")
            self.main_window.set_status_text(f"{_('Menampilkan hasil awal')} {entity} {_('untuk')} \"{query_ctx}\"...")
    ...
    def handle_search_finished(self, result_type, total_count):
        ...
        entity = _("video") if result_type == "video" else _("playlist")
        self.main_window.set_status_text(f"{_('Pencarian selesai. Ditemukan total')} {total_count} {_('hasil')} {entity}.")
        self.main_window.update_window_title_status(f"{_('Hasil Pencarian')} {entity.capitalize()}")

    def start_playlist_item_fetch_via_url(self, playlist_url):
        if any(
            (t and t.isRunning())
            for t in [self.main_window.download_thread, self.main_window.search_thread, self.main_window.channel_fetch_thread]
        ):
            QMessageBox.information(self.main_window, _("Operasi Berjalan"), _("Selesaikan operasi lain."))
            return
        ...
        self.main_window.search_results = []
        self.main_window.related_seed_url = None
        self.main_window.current_results_context = "playlist_items"

        self.main_window.set_ui_busy_state(True, "playlist_fetching")
        self.main_window.set_status_text(f"{_('Memuat item dari playlist')}: {playlist_url[:50]}...")
        self.main_window.update_window_title_status(f"{_('Memuat Isi Playlist')} ({playlist_url[:30]}...)")
        self.main_window.current_video_title_for_window = playlist_url
        if self.main_window.operation_progress_dialog:
            self.main_window.operation_progress_dialog.close()
        self.main_window.operation_progress_dialog = OperationProgressDialog(
            f"{_('Memuat Isi Playlist')}: {playlist_url[:40]}...", self.main_window
        )
        self.main_window.operation_progress_dialog.show()
    ...
    def start_channel_item_fetch(self, channel_url_or_query):
        if any(
            (t and t.isRunning())
            for t in [self.main_window.download_thread, self.main_window.search_thread, self.main_window.playlist_fetch_thread]
        ):
            QMessageBox.information(self.main_window, _("Operasi Berjalan"), _("Selesaikan operasi lain."))
            return
        ...
        self.main_window.search_results = []
        self.main_window.related_seed_url = None
        self.main_window.current_results_context = "channel_items"

        self.main_window.set_ui_busy_state(True, "channel_fetching")
        self.main_window.set_status_text(f"{_('Memuat video dari channel')}: {channel_url_or_query[:50]}...")
        self.main_window.update_window_title_status(f"{_('Memuat Channel')} ({channel_url_or_query[:30]}...)")
        self.main_window.current_video_title_for_window = channel_url_or_query
        if self.main_window.operation_progress_dialog:
            self.main_window.operation_progress_dialog.close()
        self.main_window.operation_progress_dialog = OperationProgressDialog(
            f"{_('Memuat Channel')}: {channel_url_or_query[:40]}...", self.main_window
        )
        self.main_window.operation_progress_dialog.show()

        cookie_params = self._get_cookie_params()
        self.main_window.channel_fetch_thread = ChannelFetchThread(channel_url_or_query, cookie_params, self.main_window)
        self.main_window.channel_fetch_thread.results_ready.connect(
            lambda e, t, u: self.handle_list_items_results(e, t, u, list_type="channel")
        )
    ...
    def handle_list_items_results(self, entries, list_title, original_list_url, list_type="playlist"):
        ...
        self.main_window.related_seed_url = None
        self.main_window.current_results_context = "channel_items" if list_type == "channel" else "playlist_items"

        list_name = _("Playlist") if list_type == "playlist" else _("Channel")
        item_name = _("video") if list_type == "channel" else _("item")
        dialog_result_type = "channel_items" if list_type == "channel" else "playlist_items"
        self.main_window.set_status_text(f"{list_name} '{list_title}' {_('berisi')} {len(entries)} {item_name}.")
        self.main_window.update_window_title_status(f"{_('Isi')} {list_name}: {list_title[:20]}...")
        self.main_window.current_video_title_for_window = list_title
        if not entries:
            QMessageBox.information(
                self.main_window,
                f"{_('Isi')} {list_name}",
                f"{list_name} '{list_title}' {_('kosong atau tidak berisi video yang dapat diakses.')}",
            )
            self.main_window.main_view_widget.input_line_edit.setFocus()
        ...
        self.active_search_results_dialog = SearchResultsDialog(
            entries,
            self.main_window,
            result_type=dialog_result_type,
            list_title_str=list_title,
            original_list_url=original_list_url,
            settings=self.main_window.settings,
        )
        self.active_search_results_dialog.action_triggered.connect(self.handle_action_from_search_dialog)
        self.active_search_results_dialog.download_all_playlist_items_requested.connect(
            self.main_window.download_handler.start_batch_download_list
        )
        self.active_search_results_dialog.show()
        self.main_window.main_view_widget.input_line_edit.setFocus()
    ...
    def handle_list_fetch_error(self, error_message):
        if self.main_window.operation_progress_dialog:
            ...
            self.main_window.operation_progress_dialog = None
        self.main_window.set_status_text(f"{_('Gagal Muat')}: {error_message}")
        QMessageBox.critical(self.main_window, _("Gagal Muat"), error_message)
        self.main_window.restore_proper_focus()

    def handle_action_from_search_dialog(self, info):
        if info and info.get("url"):
            action = info.get("action", "download_video")
            url = info["url"]
            title = info["title"]
            item_type = info.get("type", "video")
            self.main_window.last_selected_search_item_url = url
            if item_type == "playlist_meta" and action == "view_playlist_items":
                self.main_window.set_status_text(f"{_('Pilihan Playlist')}: {title}. {_('Memuat item...')}")
            ...
            if item_type == "video":
                if action == "play_video":
                    ...
                    self.main_window.last_downloaded_item_info = {
                        "url": url,
                        "title": title,
                        "type": "video" if action == "download_video" else "audio",
                    }
                    self.main_window.download_initiated_from_search_dialog = True
                    self.main_window.download_handler.start_download(
                        url, video_title_hint=title, download_type="video" if action == "download_video" else "audio"
                    )
                else:
                    self.main_window.set_status_text(_("Aksi tidak diketahui."))
                    self.main_window.update_window_title_status(_("Siap"))
        else:
            self.main_window.set_status_text(_("Aksi dibatalkan."))
            self.main_window.update_window_title_status(_("Siap"))
            self.main_window.restore_proper_focus()
    ...
    def handle_search_error(self, error_message):
        if self.main_window.operation_progress_dialog:
            self.main_window.operation_progress_dialog.reject()
            self.main_window.operation_progress_dialog = None
        self.main_window.set_status_text(f"{_('Kesalahan Cari')}: {error_message}")
        QMessageBox.critical(self.main_window, _("Kesalahan Cari"), error_message)
        self.main_window.restore_proper_focus()
